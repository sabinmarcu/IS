...
try  
  ...
  compilers.less = (module, filename) ->
    ...
    less.render content, (e, content) ->
      ...
      module._compile "s = document.createElement('style'); s.innerHTML = \"#{content}\"; document.head.appendChild(s); ", filename
catch err

try  
  compilers.css = (module, filename) ->
    content = fs.readFileSync filename, 'utf8'
    content = content.replace /["]/g, "'"
    content = content.replace /[\r]/g, ""
    content = content.replace /[\n]/g, "\\n"
...
try
  compilers.json = (module, filename) ->
    ...
    content = content.replace /[\r]/g, ""
    content = content.replace /[\n]/g, "\\n"
    name    = filename.substr filename.lastIndexOf "/"
    name    = name.substr 1, name.indexOf(".") - 1
    console.log name, content
    module._compile "if (!window.JSONImport) window.JSONImport = {}; window.JSONImport['#{name}'] = module.exports = JSON.Parse(#{content}); "
catch error
    ...
    fs.realpath path, (err, sourcePath) =>
      ...
      async.map @paths, fs.realpath, (err, expandedPaths) ->
        ...
        for expandedPath in expandedPaths
          base = expandedPath + ( if process.platform.indexOf("win") == 0 then "\\" else "/")
